...
//		while ((skbo = skb_dequeue(&x25->fragment_queue)) != NULL) {
//			skb_pull(skbo, (x25->neighbour->extended) ? X25_EXT_MIN_LEN : X25_STD_MIN_LEN);
////			skb_copy_from_linear_data(skbo, skb_put(skbn, skbo->len), skbo->len);
//			kfree_skb(skbo);
//		};
 ...
 * Handling of state 0 and connection release is in af_x25.c.
 */
//static int x25_state3_machine(struct sock *sk,
//							  struct sk_buff *skb,
//							  int frametype,
//							  int ns,
//							  int nr,
...
//		default:
//			pr_debug("unknown %02X in state 3\n", frametype);
//			break;
//	}
