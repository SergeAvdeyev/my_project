 ...
 */
void lapb_send_iframe(struct lapb_cb *lapb, struct lapb_buff *skb, int poll_bit) {
	unsigned char *frame = skb->data + 2;
...
void lapb_transmit_buffer(struct lapb_cb *lapb, struct lapb_buff *skb, int type) {
	unsigned char *ptr;
	...
	lapb->callbacks->debug(lapb, 2, "S%d TX %02X %02X %02X\n", lapb->state, skb->data[0], skb->data[1], skb->data[2]);

	lapb_data_transmit(lapb, skb);
	free(skb->data);
}
