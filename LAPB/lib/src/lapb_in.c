...
//		case LAPB_SABM:
//		case LAPB_SABME:
//			lapb->callbacks->debug(2, "[LAPB] S2 RX {SABM,SABME}(%d)", frame->pf);
//			lapb->callbacks->debug(2, "[LAPB] S2 TX DM(%d)", frame->pf);
//			lapb_send_control(lapb, LAPB_DM, frame->pf, LAPB_RESPONSE);
//			lapb_reset(lapb, LAPB_STATE_0);
//			break;

		case LAPB_DISC:
			lapb->callbacks->debug(2, "[LAPB] S2 RX DISC(%d)", frame->pf);
			lapb->callbacks->debug(2, "[LAPB] S2 TX UA(%d)", frame->pf);
			lapb_send_control(lapb, LAPB_UA, frame->pf, LAPB_RESPONSE);
			break;

		case LAPB_UA:
			lapb->callbacks->debug(2, "[LAPB] S2 RX UA(%d)", frame->pf);
			lapb_requeue_frames(lapb);
		...
		case LAPB_DM:
			lapb->callbacks->debug(2, "[LAPB] S2 RX DM(%d)", frame->pf);
			lapb_requeue_frames(lapb);
...
//		case LAPB_RNR:
//		case LAPB_RR:
//			lapb->callbacks->debug(2, "[LAPB] S2 RX {I,REJ,RNR,RR}(%d)", frame->pf);
//			lapb->callbacks->debug(2, "[LAPB] S2 TX DM(%d)", frame->pf);
//			if (frame->pf)
//				lapb_send_control(lapb, LAPB_DM, frame->pf, LAPB_RESPONSE);
//			break;
...
void lapb_state4_machine(struct lapb_cs * lapb, struct lapb_frame * frame) {
	...
	switch (frame->type) {
		/* Command */
		case LAPB_SABM:
			lapb->callbacks->debug(2, "[LAPB] S4 RX SABM(%d)", frame->pf);
			if (lapb_is_extended(lapb)) {
				lapb->callbacks->debug(2, "[LAPB] S4 TX DM(%d)", frame->pf);
				lapb_send_control(lapb, LAPB_DM, frame->pf, LAPB_RESPONSE);
				lapb_requeue_frames(lapb);
				lapb_disconnect_indication(lapb, LAPB_REFUSED);
				lapb_reset(lapb, LAPB_STATE_0);
			} else {
				lapb->callbacks->debug(2, "[LAPB] S4 TX UA(%d)", frame->pf);
				lapb_send_control(lapb, LAPB_UA, frame->pf, LAPB_RESPONSE);
				...
				lapb_int->vr        = 0;
				lapb_int->va        = 0;
				lapb->callbacks->debug(1, "[LAPB] S4 -> S3");
				//lapb_connect_indication(lapb, LAPB_OK);
			};
			break;
		...
		/* Command */
		case LAPB_SABME:
			lapb->callbacks->debug(2, "[LAPB] S4 RX SABME(%d)", frame->pf);
			if (lapb_is_extended(lapb)) {
				lapb->callbacks->debug(2, "[LAPB] S4 TX UA(%d)", frame->pf);
				lapb_send_control(lapb, LAPB_UA, frame->pf, LAPB_RESPONSE);
				lapb_restart_t201timer(lapb);
		...
		/* Command */
		case LAPB_DISC:
			lapb->callbacks->debug(2, "[LAPB] S4 RX DISC(%d)", frame->pf);
			lapb->callbacks->debug(2, "[LAPB] S4 TX UA(%d)", frame->pf);
			lapb_send_control(lapb, LAPB_UA, frame->pf, LAPB_RESPONSE);
			lapb_requeue_frames(lapb);
			lapb_disconnect_indication(lapb, LAPB_OK);
		...
		/* Response */
		case LAPB_DM:
			lapb->callbacks->debug(2, "[LAPB] S4 RX DM(%d)", frame->pf);
			lapb_requeue_frames(lapb);
			lapb_disconnect_indication(lapb, LAPB_NOTCONNECTED);
			lapb_reset(lapb, LAPB_STATE_0);
			break;
		...
		/* Response */
		case LAPB_FRMR:
			lapb->callbacks->debug(2, "[LAPB] S4 RX FRMR(%d)", frame->pf);
			break;
	};
}
